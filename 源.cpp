...
public:
	...
	void updateParticle(int i)
	{
		...
		if (fitnessFunction(particles[best2].pBest) > fitnessFunction(particles[(i + 5) % m].pBest))
		{
			best2 = (i + 1) % m;
		}
		if (fitnessFunction(particles[best].pBest) > fitnessFunction(particles[best2].pBest))
			best = best2;
